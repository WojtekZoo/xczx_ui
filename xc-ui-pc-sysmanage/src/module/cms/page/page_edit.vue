  ...
  <div>
    ...
    <el-form :model="pageForm" ref="pageForm" :rules="rules" label-width="80px">
      ...
      <el-form-item label="创建时间">
        <el-date-picker :disabled="true" type="datetime" placeholder="选择时间" v-model="pageForm.pageCreateTime"></el-date-picker>
      </el-form-item>
    </el-form>
...
<script>
    ...
    export default {
        methods: {
                ...
                this.$refs.pageForm.validate((valid) => {
                    if (valid) {
                        this.$confirm('确定修改?', '提示', {
                            ...
                            cmsApi.page_editById(this.$route.params.pageId, this.pageForm).then((res) => {
                                if (res.success) {
            ...
            findById(val) {
                //根据主键查询页面信息
                cmsApi.page_findById(val).then((res) => {
                    if (res.success) {
        ...
        created() {//vue 生命周期钩子函数 ， dom对象渲染完成后执行
            ...
            this.findById(this.$route.params.pageId);
        },
